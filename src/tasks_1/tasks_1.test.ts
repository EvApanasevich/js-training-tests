import {getBanknoteList, getSum, getTriangleType, isEvenIndexSumGreater, isSquareGreater, sum,} from "./tasks_1";
...
test("get banknote list", ()=> {
    ...
    const result1500 = getBanknoteList(2000)
    const result23 = getBanknoteList(56)
    expect(result1500[0]).toBe(1000)
    expect(result1500[1]).toBe(1000)
    expect(result23[0]).toBe(50)
    expect(result23[1]).toBe(5)
